...
public class ExpressionParser {
    ...
    public Node parseExpression() {
        ...
        if (cursor < string.length() - 1) {
            throw new IllegalArgumentException(String.format(
                    "(Expression parsing error) Unexpected characters '%s' at %d in expression '%s'.",
    ...
    private Node parse() throws IllegalArgumentException {
        Token token;
        ...
        if (token != null && token.start == cursor) {
            ...
            if (operatorToken != null && operatorToken.start == cursor) {
                ...
                throw new IllegalArgumentException(String.format(
                        "(Expression parsing error) Unexpected char at %d instead of operator: '%s'",
                        cursor,
            ...
            if (closedBracketToken != null && closedBracketToken.start == cursor) {
                ...
                throw new IllegalArgumentException(String.format(
                        "(Expression parsing error) Unexpected char at %d instead of closed bracket: '%s'",
                        cursor,
                        string.charAt(cursor)
                ));
            }
            ...
            if (operatorType == null) {
                throw new IllegalArgumentException(String.format(
                        "(Expression parsing error) Unknown operator at %d: '%s'",
                        operatorToken.start,
                        operatorString
                ));
            }
        ...
        throw new IllegalArgumentException(String.format(
                "(Expression parsing error) Unexpected char at %d: '%s'",
                cursor,
                string.charAt(cursor)
        ));
    }
